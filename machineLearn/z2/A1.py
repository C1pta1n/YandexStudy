...
def generate_sample(char_rnn, seed_phrase=None, max_length=500, temperature=1.0, device='cpu'):
    ...
    print(f"Generated phrase length: {len(generated_text)}")
    assert len(generated_text) == max_length, f"Generated text length is {len(generated_text)} instead of {max_length}"
...
for phrase in generated_phrases:
    ...
    if len(phrase) != 500:
        raise ValueError(f"The `generated_phrase` length should be equal to 500, but got {len(phrase)}")
